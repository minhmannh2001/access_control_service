...
def create_app(config=None):
    ...
    flask_app.config['SECRET_KEY'] = app_config.get("SECRET_KEY")

    flask_app.config["JWT_PUBLIC_KEY"] = app_config.get("PUBLIC_KEY")
    flask_app.config["JWT_PRIVATE_KEY"] = app_config.get("PRIVATE_KEY")
...
def configure_databases(app):
    configure_redis_connection(app)
    configure_postgres_connection(app)


def configure_redis_connection(app):
